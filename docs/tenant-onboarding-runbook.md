...
| T-2 | **No `admin-provision-tenant` pipeline** — ⚠️ PARTIALLY CLOSED 2026-07-24: the **Platform Console** (`docs/platform-console-runbook.md`, D-P1) now creates tenant → store → owner → till+pairing code in-app (no SQL), replacing the manual path for the CONTROL-PLANE part. Still missing: the **fiskaly fiscal-tree provisioning** leg (TEST+LIVE resource trees, §7.5) — a console-created tenant cannot fiscally issue until that lands | fiskaly leg: before tenant #2 sells (Phase 5) | Console-created tenants look complete but have no fiscal resources — the gate/checkout will fail loudly at first issuance |
