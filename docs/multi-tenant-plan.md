...
| — | **Code-signing** | **DECIDED 2026-07-05: v1 ships UNSIGNED.** Tills are Windows/Linux (Windows = one-time "Run anyway" at vendor install; Linux = no signing). No macOS tills (Mac is dev-only). | Revisit only when the fleet is large enough that manual updates hurt OR auto-update (electron-updater) is wanted — not a v1 gate | Scary first-install warning on Windows (mitigated: vendor installs, one-time); no smooth auto-update (already deferred, U2) |
...
- **Staging environment:** create the staging Supabase project (paired with production); per-environment project refs and secrets management (edge secrets, Vault keys, GROUP key TEST/LIVE split); staging Vercel target; Electron runtime-config points tills at either environment without rebuilds (**correction 2026-07-24: NOT shipped in Phase 0** — Phase 0 delivered only the DB hardening scope; the runtime-config layer is still unbuilt, tracked as S22 in `docs/PHASE-STATUS.md` and ships with `docs/update-policy.md` §10 Stage 0).
...
- **Fix the known-broken release CI** before the fleet grows — **status 2026-07-24: largely DONE.** `.github/workflows/build.yml` was rewritten (Windows-only): it injects `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON` from repo secrets (fails loudly when absent) and runs a real `npx electron-builder --win --x64 --publish=never` job that uploads the `.exe` artifact. Still open: the local `npm run electron:dist:win` script remains an echo stub (CI no longer uses it), and builds are **unsigned** — note there is no `CSC_IDENTITY_AUTO_DISCOVERY=false` flag anywhere in the current build config (that flag existed only in the pre-rewrite workflow); builds are unsigned simply because no signing config exists. Code-signing DECIDED 2026-07-05: v1 ships unsigned (see Deferral Register).
...
> **Execution status (2026-07-05): IN PROGRESS — staging identity path verified.** Staging migrations are current through `20260712000000`: the employee credential cutover plus fixes for successful-PIN SQL ambiguity and authenticated employee-profile reload. `pair-device`, JWT-only `upload-image`, and JWT-protected `manage-devices` are deployed to staging. Browser E2E passed for pair → PIN login → reload → admin Tills console → create text/QR code → copy → revoke API; temporary fixtures were fully removed. Production remains unchanged and Shim R2 remains open pending the coordinated production cutover and remaining staging authorization checks. Detailed evidence: `docs/phase2-identity.md`.

- `pair-device` edge fn + real `DevicePairing.tsx` (pre-auth route in `src/App.tsx`); tenant-admin `/devices` console + `manage-devices` edge function (admin PIN re-auth under device JWT; service-role remains server-only); device auth users; Dexie name scoping in `resolveDexieDbName()` + legacy alias for the production till (R3); post-pairing bootstrap replaces startup seed; `/order-status` pairing gate (§10).
- `employee_pin_login` + `employee_credentials` population + `LoginForm2`/`SupabaseAuthContext` rewrite **in one deploy** with stripping credentials from `get_employees_delta` (ends R2); tenant-derived `get_employee_profile` restores operator attribution on reload without direct table access. `proof_hash` deleted from `upload-image` (→`verify_jwt=true`) and `extract-purchase-document`.
- Sync services require a session (fail-closed); `ConnectivityGate` v1; **pair the production till**.
- *Files:* `supabase/functions/pair-device/`, `supabase/functions/manage-devices/`, `src/pages/DevicePairing.tsx`, `src/pages/Devices.tsx`, `src/services/deviceManagementService.ts`, `src/App.tsx`, `src/lib/localDatabase.ts:48-57`, `src/contexts/SupabaseAuthContext.tsx`, `src/components/Auth/LoginForm2.tsx`, `src/services/employeeService.ts`, `supabase/functions/upload-image/`, `supabase/functions/extract-purchase-document/`.
