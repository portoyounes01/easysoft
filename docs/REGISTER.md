...
| **A6** | The fiskaly account/org is the **SIGN PT** product | Probe resource model matched PT (UNIT/BRANCH/FISCAL_DEVICE) | OPEN | ⚠️ The TEST token JWT shows `azp: sign-es`. Likely a shared OIDC client, but **confirm the org is a PT tree, not Spanish**, before provisioning. |
