...
| **A6** | The fiskaly account/org is the **SIGN PT** product | Probe resource model matched PT (UNIT/BRANCH/FISCAL_DEVICE) | ⚠️ OPEN — evidence leans **SIGN ES** | TEST token has `azp: sign-es`; the token's own `organization`+`subject` ids **404** on the SIGN PT API (`test.api.fiskaly.com`, `2026-06-01`) though PT list endpoints return `200 {results:[]}`. **Blocks D10 provisioning.** Action (user): confirm in the fiskaly dashboard whether this workspace/API key is **Portugal (SIGN PT)**; if it's Spain, create a SIGN PT workspace + TEST API key. |
