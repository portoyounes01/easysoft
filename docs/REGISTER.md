...
| **D12** | Revoke anon **EXECUTE** on sync RPCs (defense-in-depth) | Smoke test (2026-07-06) found several sync RPCs still **anon-executable**: `get_customers_delta` → `200 []`, `upsert_products` → `200 0`, likely `upsert_categories`/`get_products_delta`. **No confirmed leak** — they self-enforce tenant so anon gets empty reads / no-op writes — and `get_employees_delta` correctly `401`s. But anon shouldn't be able to invoke write/read sync RPCs at all. | ✅ Safe to fix now that the client gates sync on a session (D6): `REVOKE EXECUTE … FROM anon` on the `get_*_delta` + `upsert_*` sync RPCs; keep `ping` anon. Add a negative test (D7). Deploy-gated on B2/B3. |
