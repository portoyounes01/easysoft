...
| **D20** | Settings/Employees fine-grained owner/admin gating (refactor Step 7) | The principal refactor (`b5214e4`) is done + verified (PWA browser login + adversarial till-path review). Step 7 — repointing Settings `isSystemAdmin` and Employees' ADMIN001 identity + role gates to grant humans the right sub-features — is NOT done. **Fails safe:** a human owner is DENIED those sub-features (no crash, no escalation), just can't fully manage until repointed. | ⏳ Do next (2 files): Settings `isSystemAdministrator(employee) \|\| principal?.role==='owner'`; Employees keep `isCurrentSystemAdmin` as an **identity** check (owner-extended) SEPARATE from a `canManageAdminEmployees` role gate — folding them together is a privilege-escalation trap (plan Gap #1). |
| **D-P2** | PWA read surfaces (reports/data) | The PWA landed on `/reports` but shows €0.00 — the browser has no data path (sync disabled §2.5 + `reportingService.getTransactionsForReporting` is missing/broken, so the server-fallback is empty). | ⏳ P2: host-aware PostgREST-direct read source for the reused pages (§2.5/§4.5) + the store dimension (D18). This is the next big PWA chunk after auth. |
