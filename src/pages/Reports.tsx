            ...
            <div className="space-y-5 md:hidden">
                <header className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex gap-2">
                        <button type="button" onClick={() => setShowMobileFilters(true)} className="relative flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500" aria-label={t('reports.header.filters')}>
                        ...
                        <button type="button" onClick={handleExportExcel} disabled={isExporting} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-xl border border-gray-200 bg-white text-gray-800 shadow-sm disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label={t('reports.header.exportExcel')}>
                ...
                {activeTab === 'overview' && <>
                    <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm" role="group" aria-label="Date range">
                        {[{ label: 'Today', days: 1 }, { label: '7 days', days: 7 }, { label: '30 days', days: 30 }].map(option => (
                            <button key={option.days} type="button" onClick={() => handleMobileDatePreset(option.days)} className={`min-h-touch-xs px-2 text-sm font-semibold transition-colors ${(mobileMetrics.periodDays === option.days || (option.days === 30 && mobileMetrics.periodDays === 31)) ? 'bg-green-600 text-white' : 'border-r last:border-r-0 border-gray-200 text-gray-600'}`}>{option.label}</button>
                        ))}
                    </div>
                    ...
                    <section>
                        ...
                        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                            {tabs.filter(tab => tab.id !== 'overview').map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setActiveTab(id)} className="flex min-h-touch-sm w-full items-center gap-3 border-b border-gray-100 px-4 text-left last:border-b-0 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"><span className="rounded-xl bg-green-50 p-2.5 text-green-700"><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-semibold text-gray-900">{label}</span><span className="block truncate text-sm text-gray-500">{id === 'employees' ? 'Sales, transactions, and productivity' : id === 'products' ? 'Top-selling products and categories' : 'Stock levels and alerts'}</span></span><ChevronRight className="h-5 w-5 text-gray-400" /></button>)}
                        </div>
                    </section>
                </>}

                {activeTab === 'employees' && <section>
                    <div className="mb-4 flex items-center gap-3"><button type="button" onClick={() => setActiveTab('overview')} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label="Back to reports"><ChevronLeft className="h-7 w-7" /></button><div><h2 className="text-2xl font-bold text-gray-900">{t('reports.tabs.employees')}</h2><p className="text-sm text-gray-500">{dateRangeLabel}</p></div></div>
                    ...
                    <h3 className="mb-2 text-xl font-bold text-gray-900">Team ranking</h3><div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">{employeePerformance.map((employee, index) => <button key={employee.employeeId} type="button" className="flex min-h-touch-sm w-full items-center gap-3 border-b border-gray-100 p-4 text-left last:border-b-0 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"><span className="w-5 font-mono text-lg font-bold text-gray-700">{index + 1}</span><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green-600 text-sm font-bold text-white">{employee.employeeName.split(' ').map(name => name[0]).join('')}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-gray-900">{employee.employeeName}</span><span className="block text-sm text-gray-500">{employee.transactionCount} transactions · {employee.itemsSold} items</span><span className="mt-2 block h-1.5 overflow-hidden rounded-full bg-gray-100"><span className="block h-full rounded-full bg-green-600" style={{ width: `${(employee.totalSales / maxEmployeeSales) * 100}%` }} /></span></span><span className="font-mono text-base font-bold text-gray-900">{formatCurrency(employee.totalSales)}</span></button>)}</div>
                </section>}

                {activeTab === 'products' && <section>
                    <div className="mb-4 flex items-center gap-3"><button type="button" onClick={() => setActiveTab('overview')} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label="Back to reports"><ChevronLeft className="h-7 w-7" /></button><div><h2 className="text-2xl font-bold text-gray-900">{t('reports.tabs.products')}</h2><p className="text-sm text-gray-500">{dateRangeLabel}</p></div></div>
                    ...
                    <h3 className="mb-2 text-xl font-bold text-gray-900">Top products</h3><div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">{mobileProducts.map((product, index) => <button key={product.productId} type="button" className="flex min-h-touch-sm w-full items-center gap-3 border-b border-gray-100 p-4 text-left last:border-b-0 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"><span className="w-5 font-mono text-lg font-bold text-gray-700">{index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-gray-900">{product.productName}</span><span className="block text-sm text-gray-500">{product.categoryName} · {product.quantitySold} {t('reports.products.table.units')}</span><span className="mt-2 block h-1.5 overflow-hidden rounded-full bg-gray-100"><span className="block h-full rounded-full bg-green-600" style={{ width: `${(product.totalRevenue / maxProductRevenue) * 100}%` }} /></span></span><span className="font-mono text-base font-bold text-gray-900">{formatCurrency(product.totalRevenue)}</span></button>)}</div>
                </section>}

                {activeTab === 'inventory' && <section>
                    <div className="mb-4 flex items-center gap-3"><button type="button" onClick={() => setActiveTab('overview')} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label="Back to reports"><ChevronLeft className="h-7 w-7" /></button><div><h2 className="text-2xl font-bold text-gray-900">Inventory health</h2><p className="text-sm text-gray-500">Prioritised by urgency</p></div></div>
