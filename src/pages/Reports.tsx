...
const ReportsInner: React.FC = () => {
    ...
    const handleExportExcel = async () => {
        ...
        try {
            ...
            setError(t('reports.error.exportFailed'));
        } finally {
            ...
            <div className="space-y-5 md:hidden">
                <header className="flex items-start justify-between gap-3">
                    {activeTab === 'overview' ? <div>
                        <h1 className="text-3xl font-bold tracking-tight text-gray-900">{t('reports.header.mobileTitle')}</h1>
                        <p className="mt-1 text-base text-gray-600">{t('reports.header.mobileSubtitle')}</p>
                    </div> : <div />}
                ...
                {activeTab === 'overview' && <>
                    <div role="group" aria-label={t('reports.filters.dateRange')}>
                        <TabToggle<string>
                            options={[{ value: '1', label: t('reports.filters.presetToday') }, { value: '7', label: t('reports.filters.presetDays7') }, { value: '30', label: t('reports.filters.presetDays30') }]}
                    ...
                    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <p className="mt-2 flex items-center gap-1.5 text-sm font-medium text-green-700"><TrendingUp className="h-4 w-4" /> {filteredTransactions.length ? t('reports.overview.currentSales') : t('reports.overview.noSalesInPeriod')}</p>
                        <div className="mt-5 h-28 border-b border-l border-dashed border-gray-200 px-1 pb-1">
                            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-full w-full" role="img" aria-label={t('reports.overview.revenueTrend')}>
                    ...
                    <div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        {[
                            ...
                            { label: t('reports.overview.avgSale'), value: formatCurrency(overviewMetrics.avgTransaction), icon: DollarSign },
                            { label: t('reports.overview.dailyAverage'), value: formatCurrency(mobileMetrics.dailyAverage), icon: TrendingUp },
                    ...
                    <section>
                        <h2 className="mb-2 text-xl font-bold text-gray-900">{t('reports.mobile.exploreReports')}</h2>
                        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                            {tabs.filter(tab => tab.id !== 'overview').map(({ id, label, icon: Icon }) => <ListRow key={id} showChevron onClick={() => setActiveTab(id)} className="focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"><span className="rounded-xl bg-green-50 p-2.5 text-green-700"><Icon className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-semibold text-gray-900">{label}</span><span className="block truncate text-sm text-gray-500">{id === 'employees' ? t('reports.mobile.employeesSubtitle') : id === 'products' ? t('reports.mobile.productsSubtitle') : t('reports.mobile.inventorySubtitle')}</span></span></ListRow>)}
                        </div>
                    </section>
                </>}

                {activeTab === 'employees' && <section>
                    <div className="mb-4 flex items-center gap-3"><button type="button" onClick={() => setActiveTab('overview')} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-2xl text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label={t('reports.mobile.backToReports')}><ChevronLeft className="h-7 w-7" /></button><div><h2 className="text-2xl font-bold text-gray-900">{t('reports.tabs.employees')}</h2><p className="text-sm text-gray-500">{dateRangeLabel}</p></div></div>
                    {leadingEmployee && <div className="mb-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm"><p className="text-sm text-gray-600">{t('reports.employees.topPerformer')}</p><p className="mt-1 text-xl font-bold text-gray-900">{leadingEmployee.employeeName}</p><p className="mt-1 text-sm text-green-700">{formatCurrency(leadingEmployee.totalSales)} · {t('reports.mobile.transactionCount', { count: leadingEmployee.transactionCount })}</p></div>}
                    <h3 className="mb-2 text-xl font-bold text-gray-900">{t('reports.mobile.teamRanking')}</h3><div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">{employeePerformance.map((employee, index) => <ListRow key={employee.employeeId} className="focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"><span className="w-5 font-mono text-lg font-bold text-gray-700">{index + 1}</span><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green-600 text-sm font-bold text-white">{employee.employeeName.split(' ').map(name => name[0]).join('')}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-gray-900">{employee.employeeName}</span><span className="block text-sm text-gray-500">{t('reports.mobile.transactionsItems', { transactions: employee.transactionCount, items: employee.itemsSold })}</span><span className="mt-2 block h-1.5 overflow-hidden rounded-full bg-gray-100"><span className="block h-full rounded-full bg-green-600" style={{ width: `${(employee.totalSales / maxEmployeeSales) * 100}%` }} /></span></span><span className="font-mono text-base font-bold text-gray-900">{formatCurrency(employee.totalSales)}</span></ListRow>)}</div>
                </section>}

                {activeTab === 'products' && <section>
                    <div className="mb-4 flex items-center gap-3"><button type="button" onClick={() => setActiveTab('overview')} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-2xl text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label={t('reports.mobile.backToReports')}><ChevronLeft className="h-7 w-7" /></button><div><h2 className="text-2xl font-bold text-gray-900">{t('reports.tabs.products')}</h2><p className="text-sm text-gray-500">{dateRangeLabel}</p></div></div>
                    {mobileProducts[0] && <div className="mb-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm"><p className="text-sm text-green-700">{t('reports.mobile.topProduct')}</p><p className="mt-1 text-xl font-bold text-gray-900">{mobileProducts[0].productName}</p><p className="mt-1 text-sm text-gray-600">{formatCurrency(mobileProducts[0].totalRevenue)} · {mobileProducts[0].quantitySold} {t('reports.products.table.units')}</p></div>}
                    <h3 className="mb-2 text-xl font-bold text-gray-900">{t('reports.mobile.topProducts')}</h3><div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">{mobileProducts.map((product, index) => <ListRow key={product.productId} className="focus:outline-none focus:ring-2 focus:ring-inset focus:ring-green-500"><span className="w-5 font-mono text-lg font-bold text-gray-700">{index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-gray-900">{product.productName}</span><span className="block text-sm text-gray-500">{product.categoryName} · {product.quantitySold} {t('reports.products.table.units')}</span><span className="mt-2 block h-1.5 overflow-hidden rounded-full bg-gray-100"><span className="block h-full rounded-full bg-green-600" style={{ width: `${(product.totalRevenue / maxProductRevenue) * 100}%` }} /></span></span><span className="font-mono text-base font-bold text-gray-900">{formatCurrency(product.totalRevenue)}</span></ListRow>)}</div>
                </section>}

                {activeTab === 'inventory' && <section>
                    <div className="mb-4 flex items-center gap-3"><button type="button" onClick={() => setActiveTab('overview')} className="flex min-h-touch-xs min-w-touch-xs items-center justify-center rounded-2xl text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label={t('reports.mobile.backToReports')}><ChevronLeft className="h-7 w-7" /></button><div><h2 className="text-2xl font-bold text-gray-900">{t('reports.inventory.mobileTitle')}</h2><p className="text-sm text-gray-500">{t('reports.mobile.prioritisedByUrgency')}</p></div></div>
                    <div className="mb-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm"><p className="text-xl font-bold text-gray-900">{inventorySummary.out + inventorySummary.low ? t('reports.mobile.itemsNeedAttention', { count: inventorySummary.out + inventorySummary.low }) : t('reports.mobile.inventoryHealthy')}</p><div className="mt-4 grid grid-cols-3 text-center"><span><b className="block text-xl text-red-600">{inventorySummary.out}</b><small className="text-gray-500">{t('reports.mobile.statusOut')}</small></span><span><b className="block text-xl text-amber-600">{inventorySummary.low}</b><small className="text-gray-500">{t('reports.mobile.statusLow')}</small></span><span><b className="block text-xl text-green-600">{inventorySummary.healthy}</b><small className="text-gray-500">{t('reports.mobile.statusHealthy')}</small></span></div></div>
                    <h3 className="mb-2 text-xl font-bold text-gray-900">{inventorySummary.out + inventorySummary.low ? t('reports.mobile.needsAction') : t('reports.mobile.healthyInventory')}</h3><div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">{inventoryProducts.filter(product => inventorySummary.out + inventorySummary.low ? product.stock <= product.min_stock : true).map(product => { const out = product.stock === 0; const low = product.stock <= product.min_stock; return <div key={product.id} className="flex min-h-touch-sm items-center gap-3 border-b border-gray-100 p-4 last:border-b-0"><span className={`rounded-xl p-2.5 ${out ? 'bg-red-50 text-red-600' : low ? 'bg-amber-50 text-amber-600' : 'bg-green-50 text-green-600'}`}>{out || low ? <AlertCircle className="h-5 w-5" /> : <CheckCircle className="h-5 w-5" />}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-gray-900">{product.name}</span><span className="block text-sm text-gray-500">{t('reports.mobile.stockMinimum', { stock: product.stock, min: product.min_stock })}</span></span><span className={`text-right text-sm font-semibold ${out ? 'text-red-600' : low ? 'text-amber-600' : 'text-green-600'}`}>{out ? t('reports.inventory.table.urgentRestock') : low ? t('reports.inventory.table.reorderSoon') : t('reports.inventory.table.noAction')}</span></div>; })}</div>
                </section>}
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-4">
                                                        ...
                                                        <div className="text-sm text-gray-500">{t('reports.employees.table.performerRank', { rank: index + 1 })}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <tr key={product.productId} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div>
                                                    ...
                                                    <div className="text-sm text-gray-500">{t('reports.products.table.bestSellerRank', { rank: index + 1 })}</div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
