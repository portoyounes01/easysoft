...
import { Bell, Loader2, Check, AlertTriangle, Percent, WifiOff } from 'lucide-react';
        ...
        <div className={`mt-5 ${off.enabled ? '' : 'pointer-events-none opacity-50'}`}>
          <label className="block max-w-[16rem]">
            ...
            <input
              ...
              onChange={(e) => setCfg((c) => ({ ...c, offline: { ...c.offline, grace_seconds: Number(e.target.value) || 0 } }))}
              className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-slate-400 focus:outline-none"
            />
          </label>
          <div className="mt-3 flex items-start gap-2 rounded-2xl bg-amber-50 px-3 py-2 text-xs text-amber-800 ring-1 ring-amber-100">
            <AlertTriangle className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" />
            <span>Till heartbeat is not shipped yet — this alert will start firing once tills report presence (a later update). The threshold is saved now.</span>
          </div>
        </div>
      </div>
